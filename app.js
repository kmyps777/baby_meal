...
const SETTINGS_REF = doc(db, 'settings', 'porridge_settings');
const RECIPES_COL  = collection(db, 'porridge_recipes');
...
window.stepValue = (id, delta) => {
  const el  = $(id);
  const min = +(el.min || 1);
  const max = +(el.max || 999);
  el.value  = Math.min(max, Math.max(min, num(id) + delta));
  calc();
};

// ── Load settings ──────────────────────────────────────────────────────────
async function loadSettings() {
  syncStatus('loading', '연결 중...');
  try {
...
function renderGrains() {
  ...
  grains.forEach((g, i) => {
    ...
    div.innerHTML = `
      ...
      <button class="del-btn" onclick="removeGrain(${i})" title="삭제">🗑️</button>
    `;
    list.appendChild(div);
  });
}

window.updateGrain = (i, key, val) => {
  grains[i][key] = val;
  calc();
...
window.stepGrain = (i, delta) => {
  const newVal = Math.max(0, (grains[i].g || 0) + delta);
  grains[i].g = newVal;
...
// ── Input listeners ────────────────────────────────────────────────────────
['n-mult','meals-per-day','days'].forEach(id =>
...
function calc() {
  ...
  grains.forEach((g) => {
    ...
    row.innerHTML = `
      ...
      <div class="ing-amount">${totalG.toFixed(0)}g</div>
    `;
...
$('save-btn').addEventListener('click', async () => {
  ...
  const memo       = $('recipe-memo').value.trim();

  const id = 'r_' + Date.now();
  const recipe = {
    id, name, n, meals, days, memo,
  ...
  try {
    await setDoc(doc(db, 'porridge_recipes', id), recipe);
    $('recipe-name').value = '';
    $('recipe-memo').value = '';
...
window.deleteRecipe = async (id) => {
  ...
  try {
    await deleteDoc(doc(db, 'porridge_recipes', id));
    syncStatus('ok', '레시피 삭제됨');
  } catch (e) {
    syncStatus('err', '삭제 실패');
  }
};
...
window.loadRecipe = (recipe) => {
  ...
  $('n-mult').value        = recipe.n;
  $('meals-per-day').value = recipe.meals;
  $('days').value          = recipe.days;
  renderGrains();
  calc();
  syncStatus('ok', `"${recipe.name}" 불러옴 ✓`);
  window.switchTab('calc');
  window.scrollTo({ top: 0, behavior: 'smooth' });
};

// ── Real-time recipes listener ─────────────────────────────────────────────
onSnapshot(RECIPES_COL, (snap) => {
  renderRecipes(snap.docs.map(d => d.data()));
}, (err) => {
  syncStatus('err', 'Firestore 오류');
  ...
  if (!list.length) {
    el.innerHTML = `
      <div class="empty-state">
        ...
        <div>저장된 레시피가 없어요<br>계산 탭에서 저장해보세요!</div>
      </div>`;
    return;
  }
      ...
      div.innerHTML = `
        <div class="rc-header">
          ...
          <div class="rc-top-right">
            ...
            <button class="rc-edit" onclick="event.stopPropagation(); openEditModal('${r.id}')" title="수정">✏️</button>
            <button class="rc-del"  onclick="event.stopPropagation(); deleteRecipe('${r.id}')" title="삭제">🗑️</button>
          </div>
        </div>
...
window.openEditModal = async (id) => {
  const snap = await getDoc(doc(db, 'porridge_recipes', id));
...
window.removeEditGrain = (i) => {
  editGrains.splice(i, 1);
  renderEditGrains();
};

window.addEditGrain = () => {
  editGrains.push({ name: '새 잡곡', g: 1 });
  renderEditGrains();
};
...
window.saveEditRecipe = async () => {
  ...
  const n          = +$('edit-n').value     || 10;
  const meals      = +$('edit-meals').value || 1;
  const days       = +$('edit-days').value  || 1;
  const totalMeals = meals * days;
  ...
  try {
    await setDoc(doc(db, 'porridge_recipes', editRecipe.id), updated);
